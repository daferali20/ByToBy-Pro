    ...
    for i in range(n_rows):
        ...
        for j in range(n_cols):
            ...
            if idx < len(df):
                ...
                row_labels.append(f"{df.iloc[idx]['symbol']}<br>{df.iloc[idx]['name']}")
            else:
...
def main():
    ...
    with col1:
        avg_change = df['change_pct'].mean() if not df.empty else 0
        st.metric("📊 متوسط التغير", f"{avg_change:.2f}%")
    with col2:
        best = df['change_pct'].max() if not df.empty else 0
        st.metric("⬆️ أفضل أداء", f"{best:.2f}%")
    with col3:
        worst = df['change_pct'].min() if not df.empty else 0
        st.metric("⬇️ أسوأ أداء", f"{worst:.2f}%")
    with col4:
        stocks_count = len(df)
        st.metric("📈 عدد الأسهم", stocks_count)
